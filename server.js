...
const { execFile, exec }require("dotenv").config();
...
async function postToInstagram(clip) {
  try {
    ...
    log("Starting Instagram Reel post for: " + clip.clipTitle, "info");

    // Step 1: Use Cloudinary to fetch and transform the YouTube video
    // into a proper publicly accessible MP4 URL
    ...
    const publicId = "clipagent_" + clip.videoId + "_" + startSecs;

    // Upload YouTube video to Cloudinary via fetch
    log("Uploading to Cloudinary via fetch: " + clip.videoId, "info");
    const cloudinary = require("cloudinary").v2;
    ...
    try {
      const uploadResult = await new Promise((resolve, reject) => {
        cloudinary.uploader.upload(
          "https://www.youtube.com/watch?v=" + clip.videoId,
          {
            ...
            eager: [
              {
                start_offset: startSecs,
                end_offset: startSecs + duration,
                width: 720,
                height: 1280,
                crop: "fill",
                aspect_ratio: "9:16",
      ...
      if (uploadResult.eager && uploadResult.eager[0]) {
        videoUrl = uploadResult.eager[0].secure_url;
      } else {
      ...
      // If upload fails (e.g. YouTube blocked), try getting existing
      log("Cloudinary upload error: " + cloudErr.message, "warn");
      // Try to use existing resource
      try {
        const existing = await new Promise((resolve, reject) => {
          cloudinary.api.resource(publicId, { resource_type: "video" }, (err, res) => {
            if (err) reject(err); else resolve(res);
          });
        });
        videoUrl = existing.secure_url;
        log("Using existing Cloudinary resource: " + videoUrl.slice(0, 80), "info");
      } catch (e) {
        log("No existing Cloudinary resource found: " + e.message, "warn");
        return false;
      }
    }

    if (!videoUrl) {
      log("No video URL available for Instagram post", "warn");
      return false;
    }
    ...
    // Step 3: Poll for processing completion (up to 3 minutes)
    let ready = false;
    for (let i = 0; i < 36; i++) {
      ...
      try {
        ...
        log("Instagram processing [" + (i+1) + "/36]: " + code, "info");
    ...
    if (!ready) {
      log("Instagram video timed out processing: " + clip.clipTitle, "warn");
      return false;
    }
...
async function postToInstagram(clip) {
  try {
    ...
    log("Uploading YouTube video to Cloudinary: " + clip.videoId, "info");

    // Step 1: Tell Cloudinary to fetch and store the YouTube video
    // Cloudinary downloads from YouTube on THEIR servers (not Railway)
    const cloudName = process.env.CLOUDINARY_CLOUD_NAME || "dj4mtzmjk";
    const apiKey = process.env.CLOUDINARY_API_KEY || "196665315695659";
    const apiSecret = process.env.CLOUDINARY_API_SECRET || "_ZJp2seaXrHEvizMpC6rVDNTvZ8";

    const youtubeUrl = "https://www.youtube.com/watch?v=" + clip.videoId;
    const publicId = "clipagent_" + clip.videoId + "_" + Date.now();
    const startOffset = clip.startSeconds || 0;
    const duration = clip.clipLength || 30;

    // Cloudinary signed upload using fetch from URL
    const crypto = require("crypto");
    const timestamp = Math.floor(Date.now() / 1000);

    // Build transformation: clip video, resize to 9:16 for Reels
    const transformation = "so_" + startOffset + ",du_" + duration + ",w_720,h_1280,c_fill,ar_9:16,vc_h264:baseline:3.1,ac_aac,br_1m";

    const sigStr = "eager=" + transformation +
      "&public_id=" + publicId +
      "&timestamp=" + timestamp +
      "&upload_preset=ml_default" +
      apiSecret;

    // Use unsigned upload with fetch_url to let Cloudinary download it
    const formData = new URLSearchParams();
    formData.append("file", youtubeUrl);
    ...
    if (!cloudRes.data || !cloudRes.data.secure_url) {
      log("Cloudinary upload failed: " + JSON.stringify(cloudRes.data).slice(0, 200), "warn");
      return false;
    }

    // Use eager transformed URL if available, else use secure_url
    let videoUrl = cloudRes.data.secure_url;
    if (cloudRes.data.eager && cloudRes.data.eager[0] && cloudRes.data.eager[0].secure_url) {
      videoUrl = cloudRes.data.eager[0].secure_url;
    }

    log("Cloudinary video ready: " + videoUrl.slice(0, 80), "success");

    // Step 2: Post as Instagram Reel
    const caption = clip.caption + "\n\n" + clip.clipTitle;
    ...
    // Step 3: Poll until Instagram finishes processing (up to 3 minutes)
    let ready = false;
    for (let i = 0; i < 36; i++) {
      await new Promise(r => setTimeout(r, 5000));
      try {
        ...
        if (code === "FINISHED") { ready = true; break; }
        if (code === "ERROR") {
          log("Instagram error: " + JSON.stringify(statusRes.data), "warn");
          return false;
        }
      } catch (e) {
    ...
    if (!ready) {
      log("Instagram timed out for: " + clip.clipTitle, "warn");
      return false;
    }
